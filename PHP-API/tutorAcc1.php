...
if (mysqli_query($conn, $sql)) {
	$sqlpassword = "INSERT INTO users (name, x, password, role) VALUES ('" . $firstname . "', '" . $email . "', '" . $password . "', '" . $verifyNo . "') ";
			if ($conn->query($sqlpassword) === TRUE) 
				 { 
				  $sqlpassword1 = "INSERT INTO  validcard (email) VALUES ('" . $email . "') ";
					   if ($conn->query($sqlpassword1) === TRUE) 
					   {
					   $sqlpasswordz = "INSERT INTO userverify (email, digit) VALUES ( '" . $email . "', '" . $verifyNo . "') ";
						  if ($conn->query($sqlpasswordz) === TRUE) 
							  { 
						    $sqledit = "INSERT INTO editprofile  (firstname, lastname, email, code) VALUES ( '" . $firstname . "','" . $lastname . "', '" . $email . "', '" . $verifyNo . "') ";
						  if ($conn->query($sqledit) === TRUE) 
							  {
							$sqlguarantor = "INSERT INTO guarantor (firstname, lastname, name, code) VALUES ( '" . $firstname . "','" . $lastname . "', '" . $email . "', '" . $verifyNo . "') ";
						  if ($conn->query($sqlguarantor) === TRUE) 
							  {}
							  }
					      //-------------------update USER---------------------
